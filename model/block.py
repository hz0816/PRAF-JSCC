...
class CCA(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv_du = nn.Sequential(
            nn.Conv2d(channel + 1, channel, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, SNR):
        b, C, _, _ = x.shape
        contrast = self.contrast(x)        # [b, C, 1, 1]
        avg      = self.avg_pool(x)        # [b, C, 1, 1]
        temp     = contrast + avg          # [b, C, 1, 1]
        if not isinstance(SNR, torch.Tensor):
            SNR = torch.tensor(SNR, device=x.device, dtype=x.dtype)
        else:
            SNR = SNR.to(x.device).type_as(x)
        snr_map = SNR.view(b, 1, 1, 1)
        temp = torch.cat([temp, snr_map], dim=1)
        y = self.conv_du(temp)
...
class PRAF(nn.Module):
    def __init__(self, in_channels, distillation_rate=0.25):
        super(PRAF, self).__init__()
    ...
    def forward(self, input, SNR):
        ...
        out_fused = self.c5(self.cca(out, SNR)) + input
        return out_fused



class PRAF_Large(nn.Module):
    def __init__(self, in_channels, distillation_rate=1 / 4):
        super(PRAF_Large, self).__init__()
        ...
        self.cca = CCA(self.distilled_channels * 6)
    ...
    def forward(self, input, SNR):
        ...
        out_c5 = self.act(self.c5(remaining_c4))
